...
::: tip 2回に分けて実施することも可能
ゼミの規模や既習度に応じて，フェーズ2を持ち帰り課題にして翌週フェーズ3〜5を行うなど，分割して実施しても構いません．
:::
...
### 教員によるテーマ再構成例（最終ディスカッション材料）

本ワークシートの元データには，教員（武）が当時，テーマ分析という方法論を意識せずに直感的に抽出した6つの概念（キーワード）が付されていました．ただしそれらは多くが **トピックの要約にとどまっており**，RTAでいうところの「現象に意味付けを与えたテーマ」までは踏み込んでいませんでした．

そこで以下では，それら6つの直感的キーワードをRTAの考え方に沿って **4つのテーマとして再構成した参考例** を示します．

::: warning これは「正解」ではない
再構成は唯一の正解ではなく，あくまで **一つの再構成のあり方** として，ゼミ生のテーマと比較するための素材として用います．
:::
...
::: details 再構成の過程について
再構成の過程で，当時のキーワード「見た目」は独立したテーマとはせず，「多感覚的癒し」と「生命との触れ合いへの没頭」に分散して位置づけました．また当初は「期待と応答のずれが顕在化させる他者性」を別テーマとして立ち上げていましたが，再検討の結果，「生命との触れ合いへの没頭」に吸収することにしました．素材の質感が静的に喚起する生命性と，応答のずれが動的に立ち上げる他者性は，いずれも「デバイスが他者として立ち現れる」という同一の現象の異なる側面と読めるためです．

**テーマの統合や分割は，RTAに含まれる本質的なプロセスであり，こうした書き直し自体がリフレクシブな分析実践の現れでもあります．**
:::

#### 議論を促す問い

- 教員の再構成例とゼミ生のテーマで， **共通する点** はどこか
- ゼミ生のテーマには出ていて，教員の再構成例には出ていない視点は何か．逆もあるか
- 教員は **作り手の視点** を持っているがゆえに，見落としていたかもしれない論点はあるか
...
::: tip 「正解はない」を強調する
RTAではテーマは構築されるものであり，唯一の正しい分類は存在しない．学生が「正解を当てる」モードに入らないよう，最初に明示的に伝える．
:::

::: tip コードを早く付けすぎないよう促す
フェーズ1で十分にデータと向き合うことが，後の分析の質を決める．「とりあえず分類しよう」とするとパターン認識が浅くなる．
:::

::: tip 教員の分類を「正解」として提示しない
対比はあくまで「視点の違いを見るための素材」．教員の分類が完成形だと受け取られると，リフレクシビティの主旨が伝わらなくなる．
:::
...
::: tip 欠損データへの対処を明示する
Q1とQ2が空欄の回答者が多いことについて，「全員が全項目に答えていない」というデータ特性をどう扱うかを学生に考えさせる機会となる．
:::

### 演習に必要な準備

- ワークシート（上記からダウンロード，PCまたは印刷版）
- 付箋（複数色あると分類作業がしやすい）
- 模造紙またはホワイトボード（テーマのグルーピング作業用）
- マーカー，ペン
